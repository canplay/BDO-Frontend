...
// Diff E:\城投\ERP\Bin E:\城投\ERP\Bin\Patch E:\城投\ERP\Bin\Update
bool DiffThread(const char* oldDir, const char* newDir, const char* outDir)
{
	...
	TiXmlElement* root = new TiXmlElement("FileList");
	...
	while (it != filelist.end())
	{
		...
		file->SetAttribute("old", it->c_str());
		file->SetAttribute("new", it->c_str());
		sprintf_s(temp, "%s.patch", it->c_str());
		file->SetAttribute("patch", temp);
		file->SetAttribute("md5", Md5::CalcFile(oldFile).c_str());

		++it;
	}

	sprintf_s(temp, "%s\\FileList.xml", outDir);
	myDocument.SaveFile(temp);

	return true;
}

// Patch E:\城投\ERP\Bin E:\城投\ERP\Bin
bool PatchThread(const char* inDir, const char* outDir)
{
	...
	for (node = node->FirstChild(); node != 0; node = node->NextSibling())
	{
		...
		sprintf_s(oldFile, "%s\\%s", inDir, Help::GetInstance().UTF8ToGBK(element->Attribute("old")).c_str());
		sprintf_s(newFile, "%s\\%s", outDir, Help::GetInstance().UTF8ToGBK(element->Attribute("new")).c_str());
		sprintf_s(patch, "%s\\Update\\%s", inDir, Help::GetInstance().UTF8ToGBK(element->Attribute("patch")).c_str());
		...
		if (element->Attribute("md5") != Md5::CalcFile(newFile))
