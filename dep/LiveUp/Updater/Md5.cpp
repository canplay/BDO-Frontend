...
void Md5::Update(MD5_CTX *context, unsigned char *input, unsigned int inputlen)
{
	...
	if (inputlen >= partlen) {
		...
		//512位为1组
		for (i = partlen;i + 64 <= inputlen;i += 64) Transform(context->state, &input[i]);
		index = 0;
	}
	else i = 0;
...
void Md5::Final(MD5_CTX *context, unsigned char digest[16])
{
	...
	Update(context, bits, 8);//这里调用时,index=0
...
std::string Md5::CalcFile(const char* szFileName)
{
	std::ifstream fin(szFileName, std::ifstream::in | std::ifstream::binary);
	if (fin)
	{
		MD5_CTX context;
		Init(&context);
		...
		while (fin.read(reinterpret_cast<char*>(buffer.get()), bufferLen))
		{
			...
			Update(&context, buffer.get(), static_cast<unsigned int>(readCount));
		}
		// 处理最后一次读到的数据
		readCount = fin.gcount();
		if (readCount > 0)
		...
		{
			Help::GetInstance().GetInstance().Log("读取文件失败");
			return 0;
		}
		...
		std::string temp = std::move(oss.str());
		transform(temp.begin(), temp.end(), temp.begin(), ::toupper);
		return temp;
	}
	else
	{
		Help::GetInstance().GetInstance().Log("无法打开文件");
		return 0;
	}
