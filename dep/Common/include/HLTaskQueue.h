	...
	public:
		...
		void release(void) {
...

			{
				...
				if (!mapDoingTask_.empty()) {
					...
					// std::unordered_map<int64_t, std::shared_ptr<HLTask>>::iterator
		...
		*/
		int deleteTask(size_t nID) {
			...
			// std::deque<std::shared_ptr<HLTask>>::iterator
		...
		*/
		int onTaskFinished(size_t nID) {
			...
			// std::unordered_map<size_t, std::shared_ptr<HLTask>>::iterator
			auto it_map = mapDoingTask_.find(nID);
